...
'''
    * Lambda 함수
        ...
        - lambda가 유용하게 사용되는 대표적 함수 (3가지)
            ...
            - map() : 각 원소를 주어진 수식에 따라서 변형하여 새로운 리스트를 반환
            - reduce() : 앞 2개의 원소를 가지고 연산.
                         연산의 결과가 또 다음 연산의 입력으로 진행된다.
                         따라서 마지막까지 진행되면 최종 출력은 한 개의 값만 남게된다.
'''
...
'''
    키가 180cm이상인 사람들의 경우 "키가 정말 큽니다."라는 메시지가 출력되고,
    그렇지 않은 경우를 "계속 키가 클겁니다."라는 메시지로 출력하는 함수를 작성하시오.

    height_high(181) => "키가 정말 큽니다."
    height_high(178) => "계속 키가 클겁니다."
'''
...
'''
    ...
    map() (함수, 입력들)
    map(function, *iterables) 형태로 사용 가능하다.
        - *iterables : 원소를 하나씩 차례로 반환 가능한 object를 의미한다.
